...
import { calcIpv4, createIpv4 } from "~/service/order"
import { getIpv4ProxyList } from "~/service/proxy"
import { noSideSpace } from "~/utils/input"
import type { FormRules } from 'naive-ui'
import type { ICountryItems } from "~/service/proxy/types"
import type { ICalcIpv4Params } from "~/service/order/types"
...
const formRef = ref()
const form = reactive<ICalcIpv4Params>({
  country: {
    id: 0,
    name: '',
    alpha2: ''
...
const initLoading = ref(!proxyStore.ipv4.country.length)
const isGetAQuote = ref(true)
const buyText = ref('Get a Quote')
const price = ref('--')
const token = ref()

const periodList = computed(() => proxyStore.ipv4.period)
const targetOptions = computed(() => proxyStore.ipv4.target)
...
const calcPrice = async () => {
  ...
  const [ err, res ] = await to(calcIpv4(toRaw(form)))
...
const createOrder = async () => {
  ...
  const [ err, res ] = await to(createIpv4(token.value, toRaw(form)))
  buyBtnLoading.value = false
  if (err) return
...
const loadIpv4ProxyList = async () => {
  await nextTick()
  if (proxyStore.ipv4.country.length) return

  initLoading.value = true
  const [ err, res ] = await to(getIpv4ProxyList())
...
onMounted(async () => {
  ...
  if (import.meta.client) {
    await loadIpv4ProxyList()
    await nextTick()

    form.country.id = proxyStore.ipv4.country[0].id
    form.country.name = proxyStore.ipv4.country[0].name
    form.country.alpha2 = proxyStore.ipv4.country[0].alpha2
    form.periodId = proxyStore.ipv4.period[0].id
            ...
            <n-gi :span="24">
              <n-form-item label="Select country" path="country.id">
                ...
                <n-select
                    ...
                    :options="proxyStore.ipv4.country"
                    label-field="country"
                    value-field="id"
                    key="id"
