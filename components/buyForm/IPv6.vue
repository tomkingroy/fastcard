...
import { getIpv6ProxyList } from "~/service/proxy"
import { calcIpv6, createIpv6 } from "~/service/order"
import { noSideSpace } from "~/utils/input"
import type { FormRules } from 'naive-ui'
import type { ICalcIpv6Params } from "~/service/order/types"
...
const formRef = ref()
const form = reactive<ICalcIpv6Params>({
  country: {
    id: 0,
    name: '',
    alpha2: ''
...
const initLoading = ref(!proxyStore.ipv6.country.length)
const isGetAQuote = ref(true)
const buyText = ref('Get a Quote')
const price = ref('--')
const token = ref()

const periodList = computed(() => proxyStore.ipv6.period)
...
const calcPrice = async () => {
  ...
  formRef.value?.validate(async (errors: any) => {
    if (!errors) {
      ...
      const [ err, res ] = await to(calcIpv6(toRaw(form)))
...
const createOrder = async () => {
  ...
  const [ err, res ] = await to(createIpv6(token.value, toRaw(form)))
...
const loadIpv6ProxyList = async () => {
  await nextTick()
  if (proxyStore.ipv6.country.length) return

  initLoading.value = true
  const [ err, res ] = await to(getIpv6ProxyList())
...
onMounted(async () => {
  ...
  if (import.meta.client) {
    await loadIpv6ProxyList()
    await nextTick()
    form.country.id = proxyStore.ipv6.country[0].id
    form.country.name = proxyStore.ipv6.country[0].name
    form.country.alpha2 = proxyStore.ipv6.country[0].alpha2
    form.periodId = proxyStore.ipv6.period[0].id
  }
})
</script>
  ...
  <ul class="buy-card">
    <li class="item characteristics">
      ...
      <ul class="desc-list">
        ...
        <li>
          <Icon class="icon" mode="svg" name="mdi-light:check-circle"/>
          <span>Randomized nets/subnets</span>
        </li>
        <li>
          <Icon class="icon" mode="svg" name="mdi-light:check-circle"/>
          <span>Supported by a small pool of websites</span>
        </li>
        <li>
          <Icon class="icon" mode="svg" name="mdi-light:check-circle"/>
      ...
      <n-spin :show="initLoading">
        ...
        >
          <n-grid :x-gap="12" :y-gap="2">
            <n-gi :span="24">
              <n-form-item label="Select country" path="country.id">
                ...
                <n-select
                    ...
                    :options="proxyStore.ipv6.country"
                    label-field="country"
                    value-field="id"
                    key="id"
            ...
            <n-gi :span="24">
              <n-form-item label="Enter target" path="customTargetName">
                <n-skeleton :sharp="false" v-if="initLoading" class="h-34"/>
                <n-input
                    v-else
                    v-model:value="form.customTargetName"
                    :allow-input="noSideSpace"
                    placeholder="Please enter the purpose of use."
                />
              </n-form-item>
            </n-gi>
